//
//  UIDevice+CSNetworkingMethods.m
//  CSNetworking
//
...
@implementation UIDevice (CSNetworkingMethods)
...
- (NSString *)CS_uuid
{
    static NSString *key = @"RTUUID";
    NSString *uuid = [[NSUserDefaults standardUserDefaults] objectForKey:key];
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self CS_createUUID] forKey:key];
...
- (NSString *)CS_udid
{
    ...
    if (udid.length==0) {
        udid = [self CS_uuid];
        [[CSUDIDGenerator sharedInstance] saveUDID:udid];
    }
    return udid;
}
...
- (NSString *)CS_macaddressMD5{
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice] CS_macaddress];
        macid = [macaddress CS_md5];
        if (!macid){
