...
// 在调用成功以后的params字典里面，获取requestID的key值
static NSString * const kCSAPIBaseManagerRequestID = @"kCSAPIBaseManagerRequestID";
...
/*************************************************************************************************/
/*                               CSAPIManagerCallBackDelegate                                    */
/*************************************************************************************************/

//api回调
@protocol CSAPIManagerCallBackDelegate <NSObject>
@required
...
*/
@protocol CSAPIManagerDataReformer <NSObject>
@required
/*
...
/*
 ...
 3.有一种情况是manager调用api时使用的参数不一定是明文传递的，有可能是从某个变量或者跨越了好多层的对象中来获得参数，那么在调用api的最后一关会有一个参数验证，当参数不对时不访问api，同时自身的errorType将会变为CSAPIManagerErrorTypeParamsError。这个机制可以优化我们的app。
 ...
 */
@protocol CSAPIManagerValidator <NSObject>
@required
...
/*************************************************************************************************/
/*                                CSAPIManagerParamSource                                        */
/*************************************************************************************************/
//让manager能够获取调用API所需要的数据
@protocol CSAPIManagerParamSource <NSObject>
...
typedef NS_ENUM (NSUInteger, CSAPIManagerRequestType){
    CSAPIManagerRequestTypeGet,
    CSAPIManagerRequestTypePost,
...
/*************************************************************************************************/
/*
 CSAPIBaseManager的派生类必须符合这些protocal
 */
...
/*************************************************************************************************/
/*                                    CSAPIManagerInterceptor                                    */
/*************************************************************************************************/
/*
 CSAPIBaseManager的派生类必须符合这些protocal
 */
@protocol CSAPIManagerInterceptor <NSObject>
@optional
- (BOOL)manager:(__kindof CSAPIBaseManager *)manager beforePerformSuccessWithResponse:(CSURLResponse *)response;
- (BOOL)manager:(__kindof CSAPIBaseManager *)manager afterPerformSuccessWithResponse:(CSURLResponse *)response;


- (BOOL)manager:(__kindof CSAPIBaseManager *)manager beforePerformFailWithResponse:(CSURLResponse *)response;
...
/*                                       CSAPIBaseManager                                        */
/*************************************************************************************************/
@interface CSAPIBaseManager : NSObject

@property (nonatomic, weak, nullable) id<CSAPIManagerCallBackDelegate> delegate;
@property (nonatomic, weak, nullable) id<CSAPIManagerParamSource> paramSource;
@property (nonatomic, weak, nullable) id<CSAPIManagerValidator> validator;
@property (nonatomic, weak, nullable) NSObject<CSAPIManager> *child;
@property (nonatomic, weak, nullable) id<CSAPIManagerInterceptor> interceptor;
...
@property (nonatomic, readonly, assign) CSAPIManagerErrorType errorType;
@property (nonatomic, strong) CSURLResponse *response;
...
/*
 ...
 CSAPIBaseManager会先调用这个函数，然后才会调用到 id<CTAPIManagerValidator> 中的 manager:isCorrectWithParamsData:
